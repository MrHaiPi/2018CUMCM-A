function ObjVal = f_2(Chrom)

for iter=1:size(Chrom,1)
dt=1;%时间间隔
dn=[3 3 18 25 250];%各层分割点数
L=10^(-3)*[0.6 Chrom(iter) 3.6 5.5 250];
dx=L./dn;%距离间隔
p=[300 862 74.2 1.18 1000];%密度
c=[1377 2100 1726 1005 2700];%比热容
k=[0.082 0.37 0.045 0.028 0.223];%热传导率
r=(dt./dx.^2).*k./(c.*p);%稳定性系数

T_environment=65;%环境温度
...
T=zeros(3600/dt+1,sum(dn)+4);%初始化温度分布
T(end,:)=T_initial;
T(:,1)=T_environment;
T(:,end)=T_skin;
...
ObjVal(iter)=0.9*T_obj(x_L(4))+0.1*Chrom(iter);
